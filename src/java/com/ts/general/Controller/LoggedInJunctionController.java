...
//            return;
//        }
        if (task.equals("showTrafficLightMap")) {
            List<History> List = clientResponderModel.showDataBean1();
            request.setAttribute("CoordinatesList", List);
            request.getRequestDispatcher("/view/general/showTrafficLightMap.jsp").forward(request, response);
            return;
        }
        }
